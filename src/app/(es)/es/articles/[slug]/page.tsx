...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildArticleMetadata(LANGUAGES.ES, slug)
}

export default async function ArticleRoute({ params }: Props) {
  const { slug } = await params
  const article = getArticleBySlug(slug)
  const { default: Content } = await loadArticleContent(slug, LANGUAGES.ES)

  return (
    <>
      <JsonLd data={buildArticleJsonLd(article, LANGUAGES.ES)} />
      <ArticlePage language={LANGUAGES.ES} article={article}>
        <Content />
      </ArticlePage>
    </>
  )
